...
public sealed class Configuration
{
    ...
    public static readonly string DefaultThemeRelativePath = Path.Combine(
        "themes",
    ...
    public Action<string, object>? LogSuccess { get; }
    ...
    )
    {
        ...
        else
        {
            if (string.IsNullOrEmpty(theme))
                theme = DefaultThemeRelativePath;
        ...
        else
        {
            Console.Error.WriteLine(
                $"{AnsiColor.Red.GetEscapeSequence()}Unable to parse '{prompt}' markup. Defaut prompt '{PromptDefault}' will be used.{AnsiEscapeCodes.Reset}"
            );
        ...
        var triggerCompletionList =
            triggerCompletionListKeyPatterns?.Any() == true
                ...
                : new KeyPressPatterns(
                    new(ConsoleModifiers.Control, ConsoleKey.Spacebar),
                    new(ConsoleModifiers.Control, ConsoleKey.J)
                );

        var newLine =
            newLineKeyPatterns?.Any() == true ? ParseKeyPressPatterns(newLineKeyPatterns) : default;
        ...
        var submitPrompt = ParseKeyPressPatterns(
            submitPromptKeyPatterns.Concat(submitPromptDetailedKeyPatterns).ToArray()
        );
        SubmitPromptDetailedKeys = ParseKeyPressPatterns(submitPromptDetailedKeyPatterns);

        var commitCompletion = new KeyPressPatterns(
            CompletionRules
                .Default.DefaultCommitCharacters.Select(c => new KeyPressPattern(c))
    ...
    internal static KeyPressPattern ParseKeyPressPattern(string keyPattern)
    {
        ...
        const string GeneralInfo =
            "Key pattern must contain one key with optional modifiers (Alt/Shift/Control). E.g. 'Enter', 'Control+A', '(', 'Alt+.', ...";
        ...
        foreach (var part in keyPattern.Split('+'))
        {
            ...
            else
            {
                throw new ArgumentException(
                    $"Unable to parse '{part}'. {GeneralInfo}",
                    nameof(keyPattern)
                );
            }
        }

        if (!(key.HasValue ^ keyChar.HasValue))
            Throw();
        ...
        else
        {
            ...
            if (modifiers != default)
                throw new ArgumentException(
                    $"Key patterns currently does not support '{keyChar.Value}' with modifiers.",
                    nameof(keyPattern)
                );
