...
internal class CSharpReplPromptCallbacks : PromptCallbacks
{
    ...
    public CSharpReplPromptCallbacks(
        IConsoleEx console,
    ...
    protected override IEnumerable<(
        KeyPressPattern Pattern,
        KeyPressCallbackAsync Callback
    )> GetKeyPressCallbacks()
    {
        yield return (
            new(ConsoleKey.F1),
            async (text, caret, cancellationToken) =>
                LaunchDocumentation(
    ...
    {
        var commitKeybinding = CreateCommitRuleForUserKeybinding(
            configuration.KeyBindings.CommitCompletion
        );
        return new CompletionItem(
            ...
            commitCharacterRules: MergeCommitRules(
                r.Item.Rules.CommitCharacterRules,
                commitKeybinding
            )
        );
    }

    private static CharacterSetModificationRule CreateCommitRuleForUserKeybinding(
        in KeyPressPatterns commitCompletion
    )
    {
        var alwaysCommitCharacters =
            commitCompletion.DefinedPatterns?.Select(key => key.Character).ToArray() ?? [];
        return new CharacterSetModificationRule(
            CharacterSetModificationKind.Add,
            ImmutableArray.Create(alwaysCommitCharacters)
        );
    }
    ...
    )
    {
        var completionRules = roslynCompletionRules
            .Select(r => new CharacterSetModificationRule(
                (CharacterSetModificationKind)r.Kind,
                r.Characters
            ))
            .ToImmutableArray();
    ...
    )
    {
        ...
        if (
            keyPress.ConsoleKeyInfo.Key == ConsoleKey.Enter
    ...
    protected override async Task<(string Text, int Caret)> FormatInput(
        string text,
        int caret,
        KeyPress keyPress,
        CancellationToken cancellationToken
    )
    {
        ...
        if (caret > 0)
        {
            switch (keyChar)
            {
                ...
                or '}':
                    return await roslyn
                        .FormatInput(text, caret, formatParentNodeOnly: false, cancellationToken)
                        .ConfigureAwait(false);
                case '{':
                    return await roslyn
                        .FormatInput(text, caret, formatParentNodeOnly: true, cancellationToken)
                        .ConfigureAwait(false);
                default:
                    break;
            }
    ...
    protected override Task<(IReadOnlyList<OverloadItem>, int ArgumentIndex)> GetOverloadsAsync(
        string text,
        int caret,
        CancellationToken cancellationToken
    ...
    )
    {
        ...
        switch (result)
        {
            case EvaluationResult.Success success:
                ...
                var output = Prompt.RenderAnsiOutput(
                    ilCode,
                    [],
                    console.PrettyPromptConsole.BufferWidth
                );
                return new KeyPressCallbackResult(text, output);
            case EvaluationResult.Error err:
                return new KeyPressCallbackResult(
                    text,
                    AnsiColor.Red.GetEscapeSequence()
    ...
    private static KeyPressCallbackResult? LaunchDocumentation(
        SymbolResult type,
        CultureInfo culture
    )
    {
        ...
        {
            LaunchBrowser(
                $"https://docs.microsoft.com/{culture.Name}/dotnet/api/{type.SymbolDisplay}"
            );
        }
