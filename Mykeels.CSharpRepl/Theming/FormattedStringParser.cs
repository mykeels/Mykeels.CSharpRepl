...
public static class FormattedStringParser
{
    ...
    public static bool TryParse(string input, out FormattedString result)
    {
        ...
        var styleMatches = StyleTagRegex
            .Matches(input)
            .Where(m =>
                StartsWithEvenNumberOf(input.AsSpan()[m.Index..], ']')
                && EndsWithEvenNumberOf(input.AsSpan(0, m.Index), '[')
            )
            .ToList();
    ...
    private static bool TryParseConsoleFormat(string input, out ConsoleFormat result)
    {
        ...
        for (int i = 0; i < parts.Length; i++)
        {
            ...
            else
            {
                ...
                {
                    if (background.HasValue || !ThemeColor.TryParseAnsiColor(part, out var color))
                    {
                        return false;
                    }
                    background = color;
                ...
                {
                    if (foreground.HasValue || !ThemeColor.TryParseAnsiColor(part, out var color))
                    {
                        return false;
                    }
                    foreground = color;
