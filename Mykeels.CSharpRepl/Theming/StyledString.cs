...
public readonly struct StyledString
{
            ...
            if (partSpan.Overlap(startIndex, length).TryGet(out var newSpan))
            {
                resultParts.Add(
                    new StyledStringSegment(
                        part.Text.Substring(newSpan.Start - i, newSpan.Length),
                        part.Style
                    )
                );
            }
    ...
    public static StyledString operator +(StyledString a, StyledString b) =>
        new(a.parts.Concat(b.parts));
}
