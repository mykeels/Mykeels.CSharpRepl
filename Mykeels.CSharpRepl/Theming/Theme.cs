...
public sealed class Theme
{
        ...
        () =>
            new(
                ...
                [
                    ...
                    new SyntaxHighlightingColor(
                        name: ClassificationTypeNames.NumericLiteral,
                        foreground: "Blue"
                    ),
                    new SyntaxHighlightingColor(
                    ...
                    new SyntaxHighlightingColor(
                        name: ClassificationTypeNames.Operator,
                        foreground: "BrightMagenta"
                    ),
                    new SyntaxHighlightingColor(
    ...
    public AnsiColor? GetSelectedCompletionItemBackgroundColor() =>
        SelectedCompletionItemBackground is null
            ? null
            : new ThemeColor(SelectedCompletionItemBackground).ToAnsiColor();

    public string? CompletionBoxBorderColor { get; }

    public ConsoleFormat? GetCompletionBoxBorderFormat() =>
        CompletionBoxBorderColor is null
            ? null
            : new ConsoleFormat(Foreground: new ThemeColor(CompletionBoxBorderColor).ToAnsiColor());

    public string? CompletionItemDescriptionPaneBackground { get; }

    public AnsiColor? GetCompletionItemDescriptionPaneBackground() =>
        CompletionItemDescriptionPaneBackground is null
            ? null
            : new ThemeColor(CompletionItemDescriptionPaneBackground).ToAnsiColor();

    public string? SelectedTextBackground { get; }

    public AnsiColor? GetSelectedTextBackground() =>
        SelectedTextBackground is null
            ? null
            : new ThemeColor(SelectedTextBackground).ToAnsiColor();
    ...
    )
    {
        ...
        syntaxHighlightingAnsiColorsDictionary = syntaxHighlightingColors.ToDictionary(
            c => c.Name,
            c => new ThemeColor(c.Foreground).ToAnsiColor()
        );
        syntaxHighlightingSpectreColorsDictionary = syntaxHighlightingColors.ToDictionary(
            c => c.Name,
            c => new ThemeColor(c.Foreground).ToSpectreColor()
        );
    }
    ...
    public AnsiColor GetSyntaxHighlightingAnsiColor(
        string? classification,
        AnsiColor defaultValue
    ) =>
        classification is null
            ? default
            : syntaxHighlightingAnsiColorsDictionary.GetValueOrDefault(
                classification,
                defaultValue
            );
    ...
    public Color GetSyntaxHighlightingSpectreColor(string? classification, Color defaultValue) =>
        classification is null
            ? default
            : syntaxHighlightingSpectreColorsDictionary.GetValueOrDefault(
                classification,
                defaultValue
            );
