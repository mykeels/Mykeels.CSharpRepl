...
internal sealed class SyntaxHighlighter
{
    ...
    public SyntaxHighlighter(MemoryCache cache, Theme theme)
    {
        ...
        this.unhighlightedSpectreColor = theme.GetSyntaxHighlightingSpectreColor(
            "text",
            Color.White
        );
    }
    ...
    internal async Task<IReadOnlyCollection<HighlightedSpan>> HighlightAsync(Document document)
    {
        ...
        if (
            this.cache.Get<IReadOnlyCollection<HighlightedSpan>>(cacheKey)
            is IReadOnlyCollection<HighlightedSpan> spans
        )
            return spans;
                ...
                var highlight =
                    classifications
                        .Select(classification =>
                            theme.GetSyntaxHighlightingAnsiColor(classification.ClassificationType)
                        )
    ...
    public AnsiColor GetAnsiColor(string? classification) =>
        theme.GetSyntaxHighlightingAnsiColor(classification, unhighlightedAnsiColor);

    public bool TryGetAnsiColor(string? classification, out AnsiColor color) =>
        theme.TryGetSyntaxHighlightingAnsiColor(classification, out color);

    public ConsoleFormat GetFormat(string? classification) =>
        new(Foreground: GetAnsiColor(classification));
    ...
    public Color GetSpectreColor(string? classification) =>
        theme.GetSyntaxHighlightingSpectreColor(classification, unhighlightedSpectreColor);

    public bool TryGetSpectreColor(string? classification, out Color color) =>
        theme.TryGetSyntaxHighlightingSpectreColor(classification, out color);

    public Style GetStyle(string? classification) =>
        new(foreground: GetSpectreColor(classification));
