...
internal sealed class DocumentationComment
{
    ...
    public static DocumentationComment FromXmlFragment(
        string? xml,
    ...
    private class CommentBuilder
    {
        ...
        /// <param name="xml">The fragment of XML to parse.</param>
        /// <returns>A DocumentationComment instance.</returns>
        public static DocumentationComment Parse(
            string xml,
            SemanticModel semanticModel,
            SyntaxHighlighter highlighter
        )
            ...
            if (_exceptionTextBuilders != null)
            {
                foreach (var typeAndBuilderPair in _exceptionTextBuilders)
                {
                    _comment._exceptionTexts.Add(
                        ...
                        typeAndBuilderPair.Value.ToImmutable()
                    );
                }
            }

            _comment.ParameterNames =
                _parameterNamesBuilder == null ? [] : _parameterNamesBuilder.ToImmutable();
            _comment.TypeParameterNames =
                _typeParameterNamesBuilder == null ? [] : _typeParameterNamesBuilder.ToImmutable();
            _comment.ExceptionTypes =
                _exceptionTypesBuilder == null ? [] : _exceptionTypesBuilder.ToImmutable();
        ...
        private void ParseCallback(XmlReader reader)
        {
            if (reader.NodeType == XmlNodeType.Element)
            {
                ...
                {
                    _comment.ExampleText = CommentInnerBuilder.Parse(
                ...
                {
                    _comment.SummaryText = CommentInnerBuilder.Parse(
                        TrimEachLine(reader.ReadInnerXml()),
                        _semanticModel,
                        _highlighter
                    );
                ...
                {
                    _comment.ReturnsText = CommentInnerBuilder.Parse(
                        TrimEachLine(reader.ReadInnerXml()),
                        _semanticModel,
                        _highlighter
                    );
                ...
                {
                    _comment.ValueText = CommentInnerBuilder.Parse(
                        TrimEachLine(reader.ReadInnerXml()),
                        _semanticModel,
                        _highlighter
                    );
                ...
                {
                    _comment.RemarksText = CommentInnerBuilder.Parse(
                        TrimEachLine(reader.ReadInnerXml()),
                        _semanticModel,
                        _highlighter
                    );
                ...
                else if (XmlNames.ElementEquals(localName, XmlNames.ParameterElementName))
                {
                    ...
                    var paramText = CommentInnerBuilder.Parse(
                        TrimEachLine(reader.ReadInnerXml()),
                        _semanticModel,
                        _highlighter
                    );
                ...
                {
                    var name = reader.GetAttribute(XmlNames.NameAttributeName);
                    var typeParamText = CommentInnerBuilder.Parse(
                        TrimEachLine(reader.ReadInnerXml()),
                        _semanticModel,
                        _highlighter
                    );
                    ...
                    {
                        (_typeParameterNamesBuilder ??= ImmutableArray.CreateBuilder<string>()).Add(
                            name
                        );
                ...
                else if (XmlNames.ElementEquals(localName, XmlNames.ExceptionElementName))
                {
                    ...
                    var exceptionText = CommentInnerBuilder.Parse(
                        reader.ReadInnerXml(),
                        _semanticModel,
                        _highlighter
                    );
                    ...
                    if (!string.IsNullOrWhiteSpace(type))
                    {
                        ...
                        {
                            (_exceptionTypesBuilder ??= ImmutableArray.CreateBuilder<string>()).Add(
                                type
                            );
                            (_exceptionTextBuilders ??= []).Add(
                                type,
                                ImmutableArray.CreateBuilder<FormattedString>()
                            );
                        }
    ...
    private class CommentInnerBuilder
    {
        ...
        public static FormattedString Parse(
            string xml,
            SemanticModel semanticModel,
            SyntaxHighlighter highlighter
        )
        ...
        private bool ParseCRef(string cref)
        {
            ...
            var displayFormat = SymbolDisplayFormat
                .MinimallyQualifiedFormat.RemoveMemberOptions(
            ...
            foreach (var part in symbol.ToDisplayParts(displayFormat))
            {
                ...
                var classification = RoslynExtensions.SymbolDisplayPartKindToClassificationTypeName(
                    part.Kind
                );
    ...
    {
        return _parameterTexts.TryGetValue(parameterName, out var text)
            ? text
    ...
    {
        return _typeParameterTexts.TryGetValue(typeParameterName, out var text)
            ? text
            : FormattedString.Empty;
    }
