...
public sealed partial class RoslynServices
{
    ...
    private readonly CSharpParseOptions parseOptions = CSharpParseOptions
        .Default.WithKind(SourceCodeKind.Script)
    ...
    [MemberNotNull(
        ...
        nameof(disassembler),
        nameof(prettyPrinter),
        nameof(symbolExplorer),
        nameof(autocompleteService),
        nameof(referenceService),
    ...
    )
    {
        ...
        try
        {
            ...
            input = await InterceptInputWhenRefStructReturnNeedsToBeHandled(
                    input.Trim(),
                    cancellationToken
                )
    ...
    public async Task<PrettyPromptTextSpan> GetSpanToReplaceByCompletionAsync(
        string text,
        int caret,
    ...
    )
    {
        ...
        if (completionService is null)
        {
            ...
            return await defaultPromptCallbacks.GetSpanToReplaceByCompletionAsync(
                text,
                caret,
                cancellationToken
            );
    ...
    public async Task<bool> ShouldOpenCompletionWindowAsync(
        string text,
        int caret,
        KeyPress keyPress,
        CancellationToken cancellationToken
    )
    {
        ...
        if (
            keyChar is '\0' or ' ' or '{' or '(' or '[' or '<' or ':' or '"'
        ...
        {
            //fallback to default PrettyPrompt implementation
            return await defaultPromptCallbacks.ShouldOpenCompletionWindowAsync(
                text,
                caret,
                keyPress,
                cancellationToken
    ...
    public async Task<bool> ConfirmCompletionCommit(
        string text,
        int caret,
        KeyPress keyPress,
        CancellationToken cancellationToken
    )
    {
        ...
        if (keyChar is ' ' or '=')
        {
            ...
            if (
                node
                is ArgumentSyntax //https://github.com/waf/CSharpRepl/issues/145
            ...
            ) // dynamic declarator https://github.com/waf/CSharpRepl/issues/231
            {
                return false;
            }
        }
        ...
        if (keyChar is ',' or ')')
        {
            ...
            if (
                ...
                is ArgumentSyntax
                    ...
                    {
                        Parent: ParenthesizedExpressionSyntax { Parent: ArgumentSyntax }
                    }
                    or IdentifierNameSyntax { Parent: ParameterSyntax }
            )
            {
                return false;
            }
    ...
    public async Task<(string Text, int Caret)> FormatInput(
        string text,
        int caret,
        bool formatParentNodeOnly,
        CancellationToken cancellationToken
    )
    {
        if (caret > 0)
        {
            ...
            document = document.WithSyntaxRoot(
                root.ReplaceToken(token, token.WithAdditionalAnnotations(annotation))
            );
            ...
            if (formatParentNodeOnly)
            {
                ...
                var span = TextSpan.FromBounds(
                    parent.FullSpan.Start,
                    Math.Min(parent.FullSpan.End, token.SpanStart)
                );
                formattedDocumentTask = Formatter.FormatAsync(
                    document,
                    span,
                    cancellationToken: cancellationToken
                );
    ...
    public Task WarmUpAsync(string[] args) =>
        Task.Run(async () =>
        {
            ...
            Task evaluationTask;
            using (
                var runtimeHelperStream = Assembly
                    .GetExecutingAssembly()
                    .GetManifestResourceStream(RuntimeHelperName)
            )
            {
            ...
            var formattingTask = FormatInput(
                @"_=""REPL Warmup"";",
                caret: 16,
                formatParentNodeOnly: false,
                default
            );
            var completionTask = Task.WhenAny(
                (await CompleteAsync(@"C", 1))
                    ...
                    .Take(15)
                    .Select(completion =>
                        completion.GetDescriptionAsync(cancellationToken: default)
                    )
            );

            await Task.WhenAll(evaluationTask, highlightTask, formattingTask, completionTask)
                .ConfigureAwait(false);
            logger.Log("Warm-up Complete");
        });

    private async Task<string> InterceptInputWhenRefStructReturnNeedsToBeHandled(
        string input,
        CancellationToken cancellationToken
    )
    {
        ...
        if (
            (
                await scriptRunner
                    .HasValueReturningStatement(input, cancellationToken)
                    .ConfigureAwait(false)
            ).TryGet(out var result)
        )
        {
            ...
            if (
                result.Type is
                { Name: "Span" or "ReadOnlySpan", ContainingNamespace.Name: "System" }
            )
            {
                wrappedExpression = Wrap(
                    nameof(__CSharpRepl_RuntimeHelper.HandleSpanOutput),
                    result.Expression
                );
            }
            else if (
                result.Type is
                { Name: "Memory" or "ReadOnlyMemory", ContainingNamespace.Name: "System" }
            )
            {
                wrappedExpression = Wrap(
                    nameof(__CSharpRepl_RuntimeHelper.HandleMemoryOutput),
                    result.Expression
                );
            }
            else if (result.Type.IsRefLikeType)
            {
                var toStringMethod = result
                    .Type.GetMembers(nameof(ToString))
                ...
                else
                {
                    expressionToBeWrapped = SyntaxFactory.LiteralExpression(
                        ...
                        SyntaxFactory.Literal(
                            $"Cannot output a value of '{result.Type.Name}' because it's a ref-struct. It has to override ToString() to see its value."
                        )
                    );
                }
                wrappedExpression = Wrap(
        ...
        static ExpressionSyntax Wrap(
            string runtimeHelperMethod,
            ExpressionSyntax expressionToBeWrapped
        ) =>
            SyntaxFactory
                ...
                .WithArgumentList(
                    SyntaxFactory.ArgumentList(
                        SyntaxFactory.SingletonSeparatedList(
                            SyntaxFactory.Argument(expressionToBeWrapped)
                        )
                    )
                );
    }
