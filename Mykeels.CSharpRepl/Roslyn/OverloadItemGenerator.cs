...
internal class OverloadItemGenerator
{
    ...
    {
        var signature = ToSignature(
            symbol,
            argumentIndex < symbolParameters.Length ? symbolParameters[argumentIndex] : null
        );
        ...
        var parameters = symbolParameters
            .Select(p => new OverloadItem.Parameter(p.Name, comment.GetParameterText(p.Name)))
            .ToArray();
    ...
    )
    {
        ...
        Debug.Assert(
            typeSymbols.All(t =>
                SymbolEqualityComparer.Default.Equals(t.ContainingSymbol, containingSymbol)
            )
        );

        var signature = ToSignature(
            containingSymbol,
            argumentIndex < typeSymbols.Length ? typeSymbols[argumentIndex] : null
        );
        var xml = containingSymbol.GetDocumentationCommentXml(cancellationToken: cancellationToken);
        var comment = DocumentationComment.FromXmlFragment(xml, semanticModel, highlighter);
        var typeParams = typeSymbols
            .Select(t => new OverloadItem.Parameter(t.Name, comment.GetTypeParameterText(t.Name)))
            .ToArray();
    ...
    private FormattedString ToSignature(ISymbol symbol, ISymbol? currentArgument)
    {
        ...
        {
            var signatureText = symbol.ToDisplayString(
                SymbolDisplayFormat.MinimallyQualifiedFormat
            );
            var currentArgumentText = currentArgument.ToDisplayString(
                SymbolDisplayFormat.MinimallyQualifiedFormat
            );
            var index = signatureText.IndexOf(currentArgumentText);
            currentArgumentSpan =
                index == -1 ? default : new TextSpan(index, currentArgumentText.Length);
        }
    ...
    )
    {
        ...
        public OverloadItem.Parameter GetTypeParameter(string name) =>
            GetParameter(TypeParameters, name);

        private static OverloadItem.Parameter GetParameter(
            IReadOnlyList<OverloadItem.Parameter> values,
            string name
        )
        {
