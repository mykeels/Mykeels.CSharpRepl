...
{
    public async Task<(
        IReadOnlyList<PrettyPromptOverloadItem> Overloads,
        int ArgumentIndex
    )> GetOverloadsAsync(string text, int caret, CancellationToken cancellationToken)
    {
        if (caret > 0)
        {
        ...
        static (IReadOnlyList<PrettyPromptOverloadItem> Overloads, int ArgumentIndex) Empty() =>
            (Array.Empty<PrettyPromptOverloadItem>(), 0);

        async Task<(
            IReadOnlyList<PrettyPromptOverloadItem> Overloads,
            int ArgumentIndex
        )> GetOverloadsForArgList(Document document, SyntaxNode argList)
        {
                ...
                foreach (var symbol in symbols)
                {
                    switch (symbol)
                    {
                        case IMethodSymbol method:
                            items.Add(
                                overloadItemGenerator.Value!.Create(
                                    method,
                                    method.Parameters,
                                    argIndex,
                                    semanticModel,
        ...
        async Task<(
            IReadOnlyList<PrettyPromptOverloadItem> Overloads,
            int ArgumentIndex
        )> GetGenericOverloadsWhenThereIsNoTypeArgListYetAsync(Document document, SyntaxNode root)
        {
            ...
            if (lessThanTokenIndex == -1)
                return Empty();

            if (
                root.FindToken(lessThanTokenIndex).Parent is BinaryExpressionSyntax
                {
            ...
            {
                if (
                    binaryExpression.Left.DescendantNodesAndSelf().Last()
                    is IdentifierNameSyntax identifierName
                )
                {
                    if (caret <= binaryExpression.OperatorToken.SpanStart)
                        return Empty();
                    ...
                    var symbols = LookupGenericMethodsAndTypes(
                        semanticModel,
        ...
        static bool TryGetArgListParent(
            SyntaxNode? node,
        ...
        static IReadOnlyList<object> GetMemberGroup(
        ...
        )
        {
            ...
            {
                return semanticModel.GetMemberGroup(
            ...
            else if (node is ElementAccessExpressionSyntax elementAccessExpression)
            {
                return semanticModel
                    .GetIndexerGroup(elementAccessExpression.Expression, cancellationToken)
                    .Cast<ISymbol>()
                    .ToImmutableArray();
            }
            ...
            {
                if (
                    constructorInitializer.ThisOrBaseKeyword.Kind()
                ...
                )
                {
                        ...
                        return baseType is null
                            ...
                            : baseType
                                .InstanceConstructors.Where(c =>
                                    semanticModel.IsAccessible(node.SpanStart, c)
                                )
                                .ToList();
                    }
                    else
                    {
                        return method
                            .ContainingType.InstanceConstructors.Where(c =>
                                semanticModel.IsAccessible(node.SpanStart, c)
                            )
                            .ToList();
                    }
                ...
                node is ImplicitObjectCreationExpressionSyntax implicitObjectCreationExpression
            )
            {
                return semanticModel.GetMemberGroup(
            ...
            else if (node is GenericNameSyntax genericNameSyntax)
            {
                ...
                var group = GetMemberGroupGeneric(
                    semanticModel,
                    genericNameSyntax,
                    cancellationToken
                );
        ...
        static IReadOnlyList<object> GetMemberGroupGeneric(
            SemanticModel semanticModel,
        ...
        )
        {
            ...
            if (node is ObjectCreationExpressionSyntax objectCreationExpression)
            {
                ...
                else if (objectCreationExpression.Type is QualifiedNameSyntax typeQualifiedName)
                {
                    ...
                    typeNamespace =
                        semanticModel
                            .GetSymbolInfo(typeQualifiedName.Left, cancellationToken)
                ...
                return semanticModel
                    .LookupNamespacesAndTypes(node.SpanStart, name: type.Identifier.ValueText)
                    .OfType<INamedTypeSymbol>()
                    .Where(t =>
                        t.IsGenericType && IsSubnamespace(t.ContainingNamespace, typeNamespace)
                    )
                    .ToArray();
            }
        ...
        static bool IsSubnamespace(INamespaceSymbol? @namespace, INamespaceSymbol? subnamespace)
        {
            while (true)
            {
                ...
                if (@namespace.Name != subnamespace.Name)
                    return false;
    ...
    private sealed class TypeArgsSequenceComparer
        : IEqualityComparer<ITypeSymbol[]>,
            IComparer<ITypeSymbol[]>
    {
