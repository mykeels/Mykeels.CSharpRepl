...
internal sealed class DotNetInstallationLocator
{
    ...
    public DotNetInstallationLocator(ITraceLogger logger)
        : this(
            ...
            userProfilePath: Path.GetFullPath(
    ...
    )
    {
        ...
        var implementationPath = GetGlobalImplementationAssemblyPath(
    ...
    private static VisualStudioInstance? LocateMsBuild(ITraceLogger logger)
    {
        try
        {
            ...
            logger.Log(
                $"Discovered dotnet version {msbuild.Name} {msbuild.Version} at {msbuild.MSBuildPath}."
            );
            return msbuild;
        }
    ...
    {
        if (!io.Directory.Exists(referenceAssemblyRoot))
            return null;

        var referenceAssemblyPath = io
    ...
    {
        if (!io.Directory.Exists(implementationAssemblyRoot))
            return null;

        var configuredFrameworkAndVersion = io
            .Directory.GetDirectories(
                implementationAssemblyRoot,
    ...
    private string? FallbackToNugetImplementationPath(string framework, Version version)
    {
        ...
        return io
            .Directory.GetDirectories(implementationPath, "net*", SearchOption.AllDirectories)
            .LastOrDefault();
    }
