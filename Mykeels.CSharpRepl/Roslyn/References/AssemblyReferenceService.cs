...
internal sealed class AssemblyReferenceService
{
    ...
    public IReadOnlySet<MetadataReference> LoadedImplementationAssemblies =>
    ...
    public AssemblyReferenceService(
    ...
    )
    {
        ...
        logger.Log(
            () => $"Implementation Assembly Paths: {string.Join(", ", implementationAssemblyPaths)}"
        );
        logger.Log(
            () =>
                $"Shared Framework Paths: {string.Join(", ", sharedFrameworkImplementationAssemblyPaths)}"
        );
        logger.LogPaths(
            "Loaded Reference Assemblies",
            () => loadedReferenceAssemblies.Select(a => a.Display)
        );
        logger.LogPaths(
            "Loaded Implementation Assemblies",
            () => loadedImplementationAssemblies.Select(a => a.Display)
        );
    }
    ...
    {
        var (referencePath, implementationPath) = dotnetInstallationLocator.FindInstallation(
            framework,
            version
        );
    ...
    )
    {
        loadedReferenceAssemblies.UnionWith(
            references
                .Select(suppliedReference => EnsureReferenceAssembly(suppliedReference))
                .WhereNotNull()
        );
    ...
    private MetadataReference? EnsureReferenceAssembly(MetadataReference reference)
    {
        ...
        if (
            cachedMetadataReferences.TryGetValue(
        ...
        var assembly =
            referenceAssemblyPaths
                ...
                .FirstOrDefault(potentialReferencePath =>
                    File.Exists(potentialReferencePath)
                    && AssemblyName.GetAssemblyName(potentialReferencePath).ToString()
        ...
        var completeMetadataReference = MetadataReference.CreateFromFile(
            assembly,
    ...
    {
        this.referenceAssemblyPaths.UnionWith(
            sharedFrameworks.Select(framework => framework.ReferencePath)
        );
        this.implementationAssemblyPaths.UnionWith(
            sharedFrameworks.Select(framework => framework.ImplementationPath)
        );
        this.sharedFrameworkImplementationAssemblyPaths.UnionWith(
            sharedFrameworks.Select(framework => framework.ImplementationPath)
        );
        this.loadedReferenceAssemblies.UnionWith(
            sharedFrameworks.SelectMany(framework => framework.ReferenceAssemblies)
        );
        this.loadedImplementationAssemblies.UnionWith(
    ...
    private List<PortableExecutableReference> CreateDefaultReferences(
        string assemblyPath,
