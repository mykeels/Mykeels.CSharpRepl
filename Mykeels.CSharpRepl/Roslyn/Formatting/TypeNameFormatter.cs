...
internal sealed partial class TypeNameFormatter
{
    ...
    public static Style GetTypeStyle(Type type, SyntaxHighlighter highlighter)
    {
        Style format;
        if (type.IsValueType)
            format = highlighter.GetStyle(ClassificationTypeNames.StructName);
        else if (type.IsInterface)
            format = highlighter.GetStyle(ClassificationTypeNames.InterfaceName);
        else if (type.IsSubclassOf(typeof(Delegate)))
            format = highlighter.GetStyle(ClassificationTypeNames.DelegateName);
        else
            format = highlighter.GetStyle(ClassificationTypeNames.ClassName);
        return format;
    }
    ...
    )
    {
        ...
        var primitiveTypeName = options.UseLanguageKeywords
        ...
        if (
            ...
            || type.IsByRef && type.GetElementType()?.IsGenericParameter == true
        )
        {
            return new StyledString(
                type.Name,
                highlighter.GetStyle(ClassificationTypeNames.TypeParameterName)
            );
        }
    ...
    private StyledString FormatNonGenericTypeName(
        TypeInfo typeInfo,
        TypeNameFormatterOptions options
    )
    {
        ...
        if (typeInfo.DeclaringType is null)
        {
            ...
            if (options.ShowNamespaces && typeInfo.Namespace != null)
            {
                ...
                {
                    namespaceParts = typeInfo.Namespace.Split(
                        '.',
    ...
    private StyledString FormatGenericTypeName(
        [MaybeNull] TypeInfo typeInfo,
        TypeNameFormatterOptions options,
    ...
    )
    {
        ...
        var genericArguments = typeInfo.IsGenericTypeDefinition
        ...
        if (typeInfo.DeclaringType != null)
        {
            ...
            do
            {
                if (IsSubmissionType(typeInfo))
                    break; //https://github.com/waf/CSharpRepl/issues/305
        ...
        else
        {
            if (options.UseLanguageKeywords)
            {
                ...
                {
                    builder.Append('(');
                    int currentArgCount = typeInfo.IsGenericTypeDefinition
        ...
        void AppendTypeInstantiation(TypeInfo typeInfo, ref int genericArgIndex)
        {
