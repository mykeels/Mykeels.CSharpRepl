...
file class ExceptionFormatter
{
    ...
    )
    {
        ...
        builder.Append(
            prettyPrinter.FormatTypeName(
                type,
                showNamespaces: fullName,
    ...
    public static StyledStringBuilder AppendMethod(
        PrettyPrinter prettyPrinter,
    ...
    )
    {
        ...
        if (method.DeclaringType != null)
        {
            ...
            else
            {
                ...
                AppendType(prettyPrinter, builder, method.DeclaringType);
                if (builderLengthBefore < builder.Length)
                    builder.Append(".");
                ...
                if (method.Name != null)
                {
                    builder.Append(
                        method.Name,
                        highlighter.GetStyle(ClassificationTypeNames.MethodName)
                    );
                }
            }
        }
        ...
        {
            if (
                parameter.ResolvedType.Assembly.ManifestModule.Name == "FSharp.Core.dll"
                && parameter.ResolvedType.Name == "Unit"
            )
                return sb;

            if (!string.IsNullOrEmpty(parameter.Prefix))
            {
                sb.Append(parameter.Prefix).Append(" ");
            }
            ...
            else if (parameter.ResolvedType != null)
            {
                ...
                {
                    IList<string>? tupleNames = null;
                    if (parameter is ValueTupleResolvedParameter { TupleNames: { } tupleNames2 })
                    {
                        tupleNames = tupleNames2;
                    }
                    ;
                    AppendType(
                        prettyPrinter,
            ...
            if (!string.IsNullOrEmpty(parameter.Name))
            {
                sb.Append(" ")
                    .Append(
                        parameter.Name,
                        highlighter.GetStyle(ClassificationTypeNames.ParameterName)
                    );
            }
