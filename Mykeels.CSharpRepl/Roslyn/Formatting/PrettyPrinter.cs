...
internal sealed partial class PrettyPrinter
{
    ...
    public FormattedObject FormatObject(object? obj, Level level)
    {
        return obj switch
        {
            ...
            CompilationErrorException compilationErrorException => new FormattedObject(
                new Paragraph(compilationErrorException.Message),
                value: null
            ),

            Exception exception => new FormattedObject(
                FormatException(exception, level).ToParagraph(),
                value: exception
            ),
    ...
    public StyledString FormatTypeName(
        Type type,
        bool showNamespaces,
        bool useLanguageKeywords,
        bool hideSystemNamespace = false,
    ...
    public StyledString FormatObjectToText(
        object? obj,
        Level level,
        bool? quoteStringsAndCharacters = null
    ) =>
        FormatObjectSafe(
            obj,
            level,
            quoteStringsAndCharacters,
            customObjectFormat: (customFormatter, obj, level, formatter) =>
                customFormatter.FormatToText(obj, level, formatter),
    ...
    public FormattedObjectRenderable FormatObjectToRenderable(object? obj, Level level) =>
        FormatObjectSafe(
            ...
            customObjectFormat: (customFormatter, obj, level, formatter) =>
                customFormatter.FormatToRenderable(obj, level, formatter),
            styledStringToResult: styledString => new FormattedObjectRenderable(
                styledString.ToParagraph(),
                renderOnNewLine: false
            ),
            styledStringSegmentToResult: styledStringSegment => new FormattedObjectRenderable(
                styledStringSegment.ToParagraph(),
                renderOnNewLine: false
            )
        );
    ...
    )
    {
        ...
        try
        {
            ...
            if (
                customObjectFormatters
                    .FirstOrDefault(f => f.IsApplicable(obj))
                    .TryGet(out var customFormatter)
            )
            {
                //custom formatters handle length limiting on it's own
                return customObjectFormat(
                    customFormatter,
                    obj,
                    level,
                    new Formatter(this, syntaxHighlighter, console.Profile)
                );
            }

            if (
                ObjectFormatterHelpers.GetApplicableDebuggerDisplayAttribute(type)?.Value is
                { } debuggerDisplayFormat
            )
            {
                var result = LengthLimiting.LimitLength(
                    FormatWithEmbeddedExpressions(debuggerDisplayFormat, obj, level),
                    level,
                    console.Profile
                );
        ...
        catch
        {
            ...
            {
                var result =
                    LengthLimiting.LimitLength(obj.ToString(), level, console.Profile) ?? "";
                return styledStringSegmentToResult(result);
            }
            catch (Exception ex)
            {
            ...
            if (c == '{')
            {
                ...
                else
                {
                    ...
                    if (
                        expressionEnd == -1
                        || (
                            memberName = ObjectFormatterHelpers.ParseSimpleMemberName(
                                format,
                                i,
                                expressionEnd,
                                out bool noQuotes,
                                out bool callableOnly
                            )
                        ) == null
                    )
                    {
                    ...
                    var member = ObjectFormatterHelpers.ResolveMember(
                        obj,
                        memberName,
                        callableOnly
                    );
                    if (member == null)
                    {
                        sb.Append(
                            GetErrorText(
                                $"{(callableOnly ? "Method" : "Member")} '{memberName}' not found"
                            )
                        );
                    }
                    else
                    {
                        var value = ObjectFormatterHelpers.GetMemberValue(
                            obj,
                            member,
                            out var exception
                        );
    ...
    private PrimitiveFormatterOptions GetPrimitiveOptions(bool quoteStringsAndCharacters) =>
        new(
    ...
    private TypeNameFormatterOptions GetTypeNameOptions(Level level) =>
        new(
            arrayBoundRadix: NumberRadix,
    ...
    public StyledString GetValueRetrievalExceptionText(Exception exception, Level level) =>
        GetErrorText(
            typeNameFormatter.FormatTypeName(exception.GetType(), GetTypeNameOptions(level))
        );
