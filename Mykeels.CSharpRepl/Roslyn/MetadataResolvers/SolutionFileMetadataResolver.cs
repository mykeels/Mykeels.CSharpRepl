...
internal sealed class SolutionFileMetadataResolver : AlternativeReferenceResolver
{
    ...
    public override async Task<ImmutableArray<PortableExecutableReference>> ResolveAsync(
        string reference,
    ...
    private async Task<ImmutableArray<PortableExecutableReference>> GetMetadataReferences(
    ...
    )
    {
        ...
        foreach (
            var error in workspace.Diagnostics.Where(d => d.Kind == WorkspaceDiagnosticKind.Failure)
        )
        {
        ...
        return projects
            .SelectMany(p =>
                p.MetadataReferences.OfType<PortableExecutableReference>()
                    .Concat(
                        p.OutputFilePath is not null
