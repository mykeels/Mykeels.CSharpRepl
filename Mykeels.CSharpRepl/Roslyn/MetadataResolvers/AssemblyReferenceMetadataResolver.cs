...
internal sealed class AssemblyReferenceMetadataResolver : IIndividualMetadataReferenceResolver
{
    ...
    )
    {
        ...
        var references = ScriptMetadataResolver
            .Default.WithSearchPaths(referenceAssemblyService.ImplementationAssemblyPaths)
        ...
        foreach (var loadedReference in references)
        {
            if (TryGetDepsJsonPath(loadedReference.FilePath, out var runtimeConfigJsonPath))
            {
                ...
                dependencyContextsPerAssemblyName.Add(
                    AssemblyLoadContext.GetAssemblyName(path).FullName,
                    new(cfg, path)
                );
            }
    ...
    private void LoadSharedFramework(PortableExecutableReference reference)
    {
        if (TryGetRuntimeConfigJsonPath(reference.FilePath, out var configPath))
        {
            ...
            if (
                JsonSerializer.Deserialize<RuntimeConfigJson>(
                    content,
    ...
    private static bool TryGetRuntimeConfigJsonPath(
        string? assemblyPath,
        [NotNullWhen(true)] out string? runtimeConfigJsonPath
    ) => TryGetAssemblyCfgPath(assemblyPath, "runtimeconfig.json", out runtimeConfigJsonPath);

    private static bool TryGetDepsJsonPath(
        string? assemblyPath,
        [NotNullWhen(true)] out string? depsConfigJsonPath
    ) => TryGetAssemblyCfgPath(assemblyPath, "deps.json", out depsConfigJsonPath);

    private static bool TryGetAssemblyCfgPath(
        string? assemblyPath,
        string extension,
        [NotNullWhen(true)] out string? runtimeConfigJsonPath
    )
    ...
    private Assembly? ResolveByAssemblyName(object? sender, ResolveEventArgs args)
    {
        ...
        located ??= referenceAssemblyService
            .ImplementationAssemblyPaths.SelectMany(path => Directory.GetFiles(path, "*.dll"))
            .Where(file => Path.GetFileNameWithoutExtension(file) == assemblyName.Name)
            .Select(loadContext.LoadFromAssemblyPath)
            .FirstOrDefault();

        if (
            located?.FullName is not null
