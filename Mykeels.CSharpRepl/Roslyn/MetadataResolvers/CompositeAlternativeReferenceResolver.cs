...
internal sealed class CompositeAlternativeReferenceResolver
{
    ...
    public CompositeAlternativeReferenceResolver(
        params AlternativeReferenceResolver[] alternativeResolvers
    )
    {
        this.alternativeResolvers = alternativeResolvers;
    }

    public async Task<ImmutableArray<PortableExecutableReference>> GetAllAlternativeReferences(
        string code,
        CancellationToken cancellationToken
    )
    {
        var splitCommands = code.Split(
            new[] { '\r', '\n' },
        ...
        return (await Task.WhenAll(resolveReferencesTasks)).SelectMany(r => r).ToImmutableArray();
    }
}
