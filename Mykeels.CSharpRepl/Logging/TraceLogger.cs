...
internal sealed class TraceLogger : ITraceLogger
{
    ...
    public void Log(string message) =>
        File.AppendAllText(path, $"{DateTime.UtcNow:s} - {message}{Environment.NewLine}");

    public void Log(Func<string> message) => Log(message());

    public void LogPaths(string message, Func<IEnumerable<string?>> paths) =>
        Log(message + ": " + GroupPathsByPrefixForLogging(paths()));
    ...
    public static ITraceLogger Create(string path)
    {
        ...
        AppDomain.CurrentDomain.UnhandledException += (_, evt) =>
            logger.Log("Unhandled Exception: " + evt.ExceptionObject.ToString());
        TaskScheduler.UnobservedTaskException += (_, evt) =>
            logger.Log("Unoberved Task Exception: " + evt.Exception.ToString());
