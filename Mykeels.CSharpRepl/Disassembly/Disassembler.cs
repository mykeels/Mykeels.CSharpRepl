...
internal class Disassembler
{
        ...
        [
            ...
            (
                ...
                compile: (code, optimizationLevel) =>
                    Compile(code, optimizationLevel, OutputKind.ConsoleApplication)
            ),
            // "DLL" will work if the user doesn't have statements, for example, they're only defining types.
            (
                name: "DLL",
                compile: (code, optimizationLevel) =>
                    Compile(code, optimizationLevel, OutputKind.DynamicallyLinkedLibrary)
            ),
    ...
    private static PlainTextOutput DisassembleFile(PEFile file)
    {
        ...
        var definedTypeNames = definedTypes
            .Select(t => asmReader.GetString(asmReader.GetTypeDefinition(t).Name))
            .ToArray();
        if (
            definedTypeNames
                .Except(["<Module>", "Program", "RefSafetyRulesAttribute", "EmbeddedAttribute"])
                .Any()
        )
        {
            new ReflectionDisassembler(ilCodeOutput, CancellationToken.None).WriteModuleContents(
                file
            ); // writes to the "ilCodeOutput" variable
            return ilCodeOutput;
        }
        ...
        var methodNames = methods
            .Select(m => asmReader.GetString(asmReader.GetMethodDefinition(m).Name))
            .ToArray();
        ...
        new MethodBodyDisassembler(ilCodeOutput, CancellationToken.None).Disassemble(
            file,
            methods[Array.IndexOf(methodNames, "<Main>$")]
        );
        return ilCodeOutput;
