...
1. ✅ **Non-Slack plumbing**: `IConsoleEx.IsInteractive`, `MessageReadEvalPrintLoop` extracted from `ReadEvalPrintLoop` (shared `PrintAsync`), `Repl.Run` branches on it and now takes an optional `console` parameter. Verified with an in-memory fake `IConsoleEx`/`IAsyncLineConsole` in `MessageReadEvalPrintLoopTests`.
2. ✅ **`SlackConsoleEx` + `SlackConsole` stub**: output buffering/formatting (`Write`/`WriteLine` → Spectre `IAnsiConsole` with `ColorSystem = NoColors` → `FlushAsync` posts one code-block reply), truncation past ~39k chars, `Inbound` channel for reads. Verified in `SlackConsoleExTests` against a fake `ISlackApiClient` (NSubstitute).
3. ✅ **`SlackReplHost`**: `SlackReplOptions`/`SlackAuthorizationContext`, `SlackAuthorizer` (fail-closed constructor, `CanStartSession`/`CanReply`), `SlackReplSession` (with `Touch()`/`LastActivityUtc` for idle timeout), and `SlackReplHost` itself — slash command handler (fast ephemeral ack, then fire-and-forget thread creation), message-event handler (bot/self filtering, session routing, per-message auth re-check), session cleanup on `Repl.Run` completion, 30s-tick idle-timeout loop. Verified via `SlackAuthorizerTests` (pure logic) and `SlackReplHostTests` (slash-command/message-event handling against a fake `ISlackApiClient`, using an internal constructor + `GetSession`/`SessionCount` test seams).
4. ✅ **Docs + sample**: `Mykeels.CSharpRepl.Sample/Program.cs` has a `slack` opt-in path (`SLACK_BOT_TOKEN`/`SLACK_APP_TOKEN`/`SLACK_ALLOWED_CHANNEL_IDS` env vars); `README.md` has a "Slack Integration" section covering Slack app setup (Socket Mode, scopes, slash command, event subscription), running the host, and the authorization options. Note the default slash command is `/mykeels-csharp-repl` (renamed from this doc's original `/new-csharp-repl` example).

**What's verified vs. not**: everything above is covered by tests that don't require a live Slack connection (33 tests total across phases 1–3, all passing) — the authorization rules, session bookkeeping, message routing, and output formatting are exercised directly. What's *not* verified, because it needs a real Slack app and can't be exercised from this environment: the actual `SlackServiceBuilder`/`ISlackSocketModeClient.Connect(...)` wiring in `SlackReplHost.Run`, whether `Connect(...)`'s returned `Task` really runs for the connection's lifetime the way `Run` assumes, and the exact ack semantics of returning a `SlashCommandResponse` over Socket Mode within Slack's 3-second window. Test this end-to-end against a real workspace before relying on it.
